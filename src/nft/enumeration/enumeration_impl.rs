...
use crate::nft::base::{NonFungibleTokenCore, NonFungibleToken};
use crate::nft::metadata::{TokenId, Token};
...
impl NonFungibleToken {
  ...
  pub fn internal_nft_tokens<F>(&self, get_token: &F, from_index: Option<U128>, limit: Option<u64>) -> Vec<Token>
    where F: Fn(AccountId, TokenId) -> Token
  {
    ...
    self.owner_by_id
      ...
      .take(limit)
      .map(|(token_id, owner_id)| get_token(owner_id, token_id))
      .collect()
  }
}
...
impl NonFungibleTokenEnumeration for NonFungibleToken {
  ...
  fn nft_tokens(&self, from_index: Option<U128>, limit: Option<u64>) -> Vec<Token> {
    let get_token = &|owner_id: AccountId, token_id: TokenId| self.enum_get_token(owner_id, token_id);
    self.internal_nft_tokens(get_token, from_index, limit)
  }
